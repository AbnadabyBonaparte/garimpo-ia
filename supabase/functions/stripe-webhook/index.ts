...
// Env: STRIPE_WEBHOOK_SECRET, STRIPE_SECRET_KEY, ALLOWED_ORIGIN
...
serve(async (req) => {
  ...
  const v1 = parts['v1'];
  if (!timestamp || !v1) return new Response('Invalid signature format', { status: 400 });

  // ── Timestamp replay attack protection ──────────────────
  const timestampAge = Math.floor(Date.now() / 1000) - parseInt(timestamp, 10);
  if (Math.abs(timestampAge) > WEBHOOK_TOLERANCE_SECONDS) {
    return new Response('Webhook expired — replay attack prevention', { status: 400 });
  }
  ...
  try {
    ...
    const expectedHex = Array.from(new Uint8Array(expected))
      ...
      .join('');
    if (v1 !== expectedHex) return new Response('Invalid signature', { status: 400 });
  } catch {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (!userId) return new Response(JSON.stringify({ received: true }), { headers: { 'Content-Type': 'application/json' } });
    ...
    const tier = priceId ? mapToTier(priceId) : null;
    if (tier) {
